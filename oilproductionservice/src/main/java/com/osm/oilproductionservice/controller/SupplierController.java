...
public class SupplierController {
    ...
    public ResponseEntity<ApiResponse<List<SupplierDto>>> getSupplierByRegion(@PathVariable Long regionId) {
        List<SupplierDto> supplier = supplierService.getSuppliersByRegion(regionId);
        if (supplier != null) {
            ApiResponse<List<SupplierDto>> response = new ApiResponse<>(true, "SupplierDto fetched successfully by region", supplier);
            return ResponseEntity.ok(response);
        } else {
            ApiResponse<List<SupplierDto>> response = new ApiResponse<>(false, "SupplierDto not found for region " + regionId, null);
            return ResponseEntity.notFound().build();
        }
    }
}
